...
Every bn-kernel workflow that starts a headless bridge owns that bridge and must apply both cleanup layers:

1. **Deterministic cleanup:** after the final read, close the exact loaded target when one exists, then stop the exact instance. Instance stop must still run if target close fails, loading fails, analysis raises, cooperative cancellation runs cleanup, or the result is partial. Hard agent/process death is handled by the idle fallback.
...
5. In unconditional cleanup, close the exact target if it was opened and always run `bn session stop ID` afterward.

If startup returns an error after creating or registering the bridge, attempt the same exact instance stop; a clean “instance not found” result means there is nothing registered under that ID. An agent must not assume a failed or timed-out start left no process, and it must not broaden cleanup beyond its unique ID.

## Fan-Out Flow

Every parallel child owns a different bridge instance. Parent prompts must state the lifecycle contract explicitly, including the one-hour fallback and unconditional exact teardown. Each child returns only a bounded analysis summary; it never returns a live `Session`, bridge ownership, or cleanup responsibility to the parent.
...
1. Run pressure scenarios against the current skill text without the new lifecycle guidance. Scenarios must combine completion pressure, a failed or timed-out start, and fan-out ownership. Record whether agents omit explicit stop, omit the idle fallback, or widen cleanup.
2. Add the smallest positive lifecycle recipe and fan-out prompt contract that address the observed failures.
3. Re-run the same scenarios with the edited skill. Passing behavior requires both `BN_IDLE_TIMEOUT=3600` at start and exact unconditional instance stop, including failure paths.
...
- A start timeout is treated as uncertain ownership, not proof that no bridge exists.
