...
def test_no_renderer_states_a_count_it_could_not_read_as_a_real_number():
    ...
    # #812: `_render_backward_diagnostics` adds five
    ...
    # #827 item 1: `_render_taint_path` adds `alternate_parents`
    ...
    # #823: `_render_spill_gc_text` states five top-level counters
    ...
    assert len(sites) == 41, (
        ...
        "count helper, not 41. The number is the size of the covered set: a "
    ...
    assert sorted(not_stated) == [
        ...
        # #827 item 1: an unreadable alternate-parent count suppresses the
        # disclosure clause. The dedicated taint-path test covers that branch.
...
def test_no_renderer_raises_on_a_field_the_absent_payload_survived():
    ...
    # Combined population: 5168 + 16 (#693) = 5184 renders.
    assert swept == 5184, f"the raise sweep ran {swept} renders, not 5184"
...
def test_the_malformed_disclosure_never_fires_on_a_well_formed_payload():
    ...
    * A POPULATED container is excluded because there is no single well-formed
    ...
    # Combined population: 1501 + 4 (#693) = 1505 renders.
    assert checked == 1505, f"the mirror ran {checked} renders, not 1505"
