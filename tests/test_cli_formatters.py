...
def test_a_present_container_is_never_absorbed_into_the_empty_rendering():
    ...
    # #890 obs 2: `_operation_row_text` now reads the `unapplied_prototypes` container and its nested `functions`/`variables` lists, so a text reader of a mixed declare is told what went unapplied. Three new discovered reads, and these derived populations grow with them. 1212 + 6 = 1218.
...
def test_no_renderer_raises_on_a_field_the_absent_payload_survived():
    ...
    # #890 obs 2: `_operation_row_text` now reads the `unapplied_prototypes` container and its nested `functions`/`variables` lists, so a text reader of a mixed declare is told what went unapplied. Three new discovered reads, and these derived populations grow with them. 4896 + 8 = 4904.
...
def test_the_malformed_disclosure_never_fires_on_a_well_formed_payload():
    ...
    * A POPULATED container is excluded because there is no single well-formed
    ...
    # #890 obs 2: `_operation_row_text` now reads the `unapplied_prototypes` container and its nested `functions`/`variables` lists, so a text reader of a mixed declare is told what went unapplied. Three new discovered reads, and these derived populations grow with them. 1426 + 3 = 1429.
