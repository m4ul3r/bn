...
def test_no_renderer_states_a_count_it_could_not_read_as_a_real_number():
    ...
    # #823 adds seven spill-GC counter reads; #795 adds eleven reads for
    # filtered strings, imports exclusions, function counts, and class lists.
    # Measured on the integrated tree: 19 + 7 + 11 = 37 pairs.
    assert len(sites) == 37, (
        f"the module reads {len(sites)} (renderer, literal key) pairs through a "
        "count helper, not 37. The number is the size of the covered set: a "
...
def test_a_present_container_is_never_absorbed_into_the_empty_rendering():
    ...
    # #797 adds the def-use hints list to default's 1236 cases: one position
    # under six malformed container shapes. Measured here: 1242 cases.
...
def test_no_renderer_raises_on_a_field_the_absent_payload_survived():
    ...
    #
    # #770 adds three paging reads; #795 adds filtered and artifact_count;
    # #797 adds hints. Those six pairs add 48 malformed renders to default's
    # 4976. The artifact_count read is observed, but its originating commit
    # is not inferred from this population. Measured here: 5024 renders.
...
def test_the_malformed_disclosure_never_fires_on_a_well_formed_payload():
    ...
    * A POPULATED container is excluded because there is no single well-formed
    ...
    #
    # The same paging/filtered/artifact_count pairs add two benign renders
    # each; the hints list adds three. Measured here: 1450 + 13 = 1463.
