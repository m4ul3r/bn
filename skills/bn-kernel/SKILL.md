...
- `await s.decompile(identifier)` returns the non-empty text string with inherited annotation bodies redacted from standalone and inline/trailing `//` comments when the complete comment body matches a stored annotation line (ignoring surrounding whitespace). Code, quoted/escaped literals, address gutters, and unrelated comments are preserved. This is not arbitrary-text scrubbing: block comments and differently rendered bodies are not redacted. Use `include_annotations=True` only after an explicit contamination decision. Skipped placeholders raise and direct you to `force_analysis=True`.
