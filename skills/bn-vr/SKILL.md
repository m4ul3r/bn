...
> **Quick-loaded target?** If the binary was opened with `bn load --quick` / `bn session start --quick`, the code is not analyzed yet: `bn imports` and `bn sections` work, but `bn strings` (step 3) and `bn function list` / `bn function search` are empty/partial — a false "no dangerous strings, no sinks" all-clear. Confirm `analysis_state` is `"full"` (`bn target info`) and `bn refresh` before auditing. See "Quick Load" in the `bn` skill.
