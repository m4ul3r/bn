...
> **Quick-loaded target?** If the binary was opened with `bn load --quick` / `bn session start --quick` (fast, no analysis), `bn imports` and `bn sections` work, but `bn strings` and `bn function list` are empty/partial until you run `bn refresh` — steps 2–3 would otherwise read as "no strings, almost no functions" and mislead the survey. Check `analysis_state` in `bn target info` (`"quick"` vs `"full"`) and `bn refresh` before surveying. See "Quick Load" in the `bn` skill.
