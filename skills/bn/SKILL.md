...
- **HLIL can mislead beyond access width — trust `bn disasm`.** Pseudo-C / HLIL distort more than the operand size. Before concluding on a bound / overflow / off-by-one, confirm the shape in `bn disasm`:
  ...
  **Exactly one of these is detectable from a listing, and `bn decompile` warns about it by name** (#676 item 12): the conditional-compare/`csel` bullet, because a branch-free conditional is an *instruction* (`ccmp`/`csel`/`cmov*`/Thumb `it*`) and its presence is self-evidencing whatever the target. The other three are not covered — access width needs the deref's context, and the hoisted bound and the missing `<< 4` are *semantic*, with no mnemonic to key on. **No note will ever fire for those three, and the absence of a warning says nothing about them.** Check them yourself; this list is the only place they are enumerated.
