...
- `function list --count` counts all functions; `function search <query> --count` counts matches. `--sort size --reverse --min-size N` is useful for large functions. `--named` / `--unnamed` separate meaningful names from auto `sub_*` names. Duplicate function starts are disclosed with `duplicate_starts_collapsed` / `duplicate_starts_unresolved` where relevant.
