...
For taint JSON, forward flows are `reached_sinks[]`, each with `.sink` and `.path`; backward results are `slices[]`, while top-level `sinks` repeats the query. `sink_status[]` discloses whether a backward sink was seeded or proven `bounded`. On a zero-sink forward run, read `diagnostics.safe_to_report_all_clear`: weak seeds or unmodeled-call assumptions can withhold an all-clear even with no leaves or truncation. A true gate only supports absence of modeled sink flow under that analysis, not absence of bugs.

Use `--models FILE` for a known wrapper; keys are current symbol names (including `sub_*` on stripped targets), never addresses, and entries nest `sink` or `sources`. For `app_recv(fd, len, buf)`, `{"app_recv":{"sink":{"class":"overflow_len","len_arg":1,"buf_arg":2}}}` models a bounded-write sink; `{"app_recv":{"sources":[{"to":"*arg:2"},{"to":"ret"}]}}` seeds its output buffer and return. `sources` is a list of `to` rules: `*arg:N` means the pointed-to buffer, `arg:N` its value, and `ret` the return. A model with no recognized role can be counted as loaded but affect nothing; confirm the rendered model and its effect.
