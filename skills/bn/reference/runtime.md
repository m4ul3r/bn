...
A headless start owns an instance that should be stopped when work ends. Set a positive `BN_IDLE_TIMEOUT` on an agent-owned start as a crash fallback (`BN_IDLE_TIMEOUT=3600` gives one hour); unset means no idle reaper. The command reports the loaded target selector. For long analysis, `--detach` registers a bridge and returns a load-job id. Poll that id: `terminal: false` means keep waiting; on terminal, `succeeded` is true or false. A no-id `session status` lists jobs, not one verdict. `status_command` is null when the bridge cannot name itself in a fresh CLI invocation. On a loaded host, raise `BN_SPAWN_TIMEOUT` (for example, `180` seconds) for registration and set the orchestration command timeout higher; this is separate from `BN_IDLE_TIMEOUT` and the detached load-job budget. Stop an exact owned instance even if its target close fails; a caller killed by its outer timeout may have left a child that registered later. Leave a pre-existing instance alone when start returns the exact duplicate-ID error.

A saved `.bndb` may be preferred over adjacent raw bytes. On a read-only source mount, `bn save` can use a content-hash cache under `BN_CACHE_DIR` (default `~/.cache/bn`); a later load can restore its annotations. Use `--no-bndb` when the task requires raw-byte analysis without prior names/comments. A save normally preserves the live target selector; if the response reports `rehomed: true`, get the new selector from `bn target list`. For post-save commands, prefer the open view's `target_id`: a stale one fails after bridge restart, while a process-local `view_id` can be reused for a different view. After restart or close/reload, list targets again: the loaded filename and IDs can change, and a raw view beside its `.bndb` can make a bare basename ambiguous.

`--quick` skips the full analysis pass. Sections and imports are useful immediately; function listings/searches are partial, strings can refuse, and xrefs, callsites, and taint need analysis. All mutations refuse a quick-loaded target until `bn refresh`. Check `analysis_state` and run `bn refresh` before using these reads for a complete survey. `decompile --force-analysis` can analyze one existing function; it does not replace full target analysis or create a missing function.
