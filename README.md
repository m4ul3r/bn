...
Any status above other than `verified`/`noop` puts a mutation at exit code `3`. Exit `4` is the separate "unverifiable" case: the call did not fail, but its compact summary reports `measured: false` because the result carried no `results[]` rows to derive counts from *and* the command registered no summary of its own to count with. The summary then also sets a fail-safe `dirty_after: true` and a `first_error` explaining that the counts are unknown. `4` is distinct from `3` (a failure, which still wins when both apply) and from `0`, so a script that only checks `$?` cannot read an unconfirmed write as a clean success. The rule is keyed on `measured: false` rather than on the kind of call, so a `--preview` that comes back unmeasured is `4` as well — nothing was applied there, and what could not be confirmed is what *would* have landed. A command that reports through its own counters instead of `results[]` — `bn go rename` is the only one today — registers a compact summary for those counters and therefore stays measured, so it exits `0` on success like any other. A measured all-`noop` is `0` too.
