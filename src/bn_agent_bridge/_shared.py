...
# Name-shape fallback for names BN's loaders/analysis can mark `auto=False`.
# This is NOT provenance: an analyst rename matching one of these families is
# indistinguishable and can be missed. BN's internal namespace is not proof
# either -- user-defined symbols can carry it too. `read_listing` prefers an
# address-specific imported-debug-info match and discloses every fallback.
_PLACEHOLDER_SYMBOL_NAME = re.compile(
    r"^(?:func(?:_[0-9a-fA-F]+)?"
    r"|sub"
    r"|(?:init|start)_routine(?:_[0-9a-fA-F]+)?"
    r"|_?init|_?fini|main|dest|(?:destr|compar)(?:_[0-9a-fA-F]+)?)$"
)


def is_placeholder_symbol_name(name: str) -> bool:
    """True when a name resembles a machine-generated symbol (a heuristic).
