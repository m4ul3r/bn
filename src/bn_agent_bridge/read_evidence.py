"""Pointer tables, RTTI/message lensing, init-array walking, hidden surfaces.

The lensing cluster over the ``ctx`` seam: ``pointer_table`` (`evidence table`,
also embedded in the init-array and message digests), ``message_lens`
(`evidence message`), ``init_arrays`` (`evidence init`), ``call_descriptor_evidence``
(`evidence descriptor`) and ``hidden_surface`` (`evidence surface`) -- plus the
virtual-call resolution `evidence vcall` builds on. All of them read code
pointers, record tables, vtables and RTTI out of the view; none of them renders a
call's arguments.

The RAW-ABI CALL-EVIDENCE cluster (`evidence function`: per-call argument
recovery, arity confidence, thunk detection) moved to ``read_call_evidence.py``
(#592). The two had zero shared functions and disjoint dependencies --
everything moved reaches ``il_format``'s call helpers, everything left reaches
``ctx``'s pointer/address helpers -- so the seam was free to take, and the
module no longer hosts two unrelated concerns.
...
Outbound calls resolve through:
  * ``ctx`` -- resolution / address-context helpers relocated to the seam
    ...
    ``_byteorder``, ``_sections_at``, ``_supports_thumb_pointer_tags``);
...
``bridge`` or ``seam`` -- those import THIS module one-way (design spec 3.2). It
imports ``read_xrefs`` but ``read_xrefs`` NEVER imports this module.
"""
...
# #592: the raw-ABI call-evidence cluster lives in ``read_call_evidence`` now, but
# these names stay importable HERE for the callers that bind them through this
# module's namespace and are not part of this change: bridge.py's delegating shims
# (the established pattern -- the op binders and the test suite bind handlers as
# ``BinaryNinjaBridge._<name>``, and each shim forwards to this module),
# ``read_decompile._thunk_veneer_warning`` and ``read_class``'s method lens (both
# look up ``read_evidence._function_thunk_summary``), and
# ``test_read_decompile``'s monkeypatch of that same attribute. Repointing them is
# a separate change (bridge.py is contended), so treat this as a TRANSITIONAL
# surface, not this module's API: new code imports from ``read_call_evidence``.
# The list is exactly the moved names those call sites reach -- nothing here
# re-exports the cluster's internals.
from .read_call_evidence import (  # noqa: F401  (transitional re-export, see above)
    _call_arguments,
    _call_destination_value,
    _cpp_method_this_caveat,
    _function_call_evidence,
    _function_evidence,
    _function_thunk_summary,
    _il_argument_texts,
    _mlil_call_text,
    _resolve_argument_value,
    _safe_int,
    _target_entry_for_call,
)
