...
- Exit codes: 0 = success, 1 = CLI-side handler error (e.g. partial `session start` failure), 2 = `BridgeError` (transport failures and bridge-side errors, including a status in `FAILED_MUTATION_STATUSES` on a read/resolver call), 3 = a `_mutate`-marked call whose status is `verification_failed`, `unsupported`, `invalid_request`, `rollback_failed`, or `internal_error` — the refusal is exit 3 whether it was raised up front or during apply, never 2 for a mutation — 4 = a `_mutate`-marked call that applied but could not be verified: its compact summary reports `measured: false` because the op returned no `results[]` rows to derive counts from (#715). A failure still wins over 4 (3 before 4), and a measured all-`noop` is 0
