...
- `mutation_statuses.py` — `FAILED_MUTATION_STATUSES`, the one definition of the mutation failure vocabulary. Split out of `formatters.py` because the set had two owners (#777): this package classified the statuses while the BRIDGE produced them from its own narrower inline copy, so the bridge's revert-on-failure gate covered two of the five and the next verifier to raise one of the other three would have left a failed batch un-reverted. `formatters.py` re-exports the name it is documented under; symlinked into `src/bn_agent_bridge/` like `paths.py`.
...
- `wire_limits.py` — the shared hard request-size ceiling (`MAX_REQUEST_BYTES`) and `batch apply` budgets. Transport checks the actual serialized batch request after it resolves the bridge identity and target, before sending it; a fixed envelope estimate cannot account for those fields. `BN_BATCH_APPLY_MAX_OPS` can raise or disable the optional op-count limit (`0` disables). `BN_BATCH_APPLY_MAX_BYTES` can set a lower local byte limit, but cannot raise or disable the bridge's hard wire limit. Symlinked into `src/bn_agent_bridge/` like `paths.py`, so the transport and bridge read one number.
- `headless.py` — `bn-agent` entry point.

`src/bn_agent_bridge/paths.py`, `version.py`, `proc_identity.py`, `socket_evidence.py`, `target_hint.py`, `wire_limits.py` and `mutation_statuses.py` are symlinks to `src/bn/`, so the bridge and CLI share the filesystem layout, version, process identity, socket evidence, target hint grammar, request-size ceiling and mutation failure-status vocabulary. `bn.formatters.FAILED_MUTATION_STATUSES` re-exports the shared status set (#777).
...
- Command handlers are named `_<group>_<subcommand>()` (e.g., `_function_list`); a top-level command keeps its bare verb (`_decompile`, `_strings`) — `help` is `_help_index`, and a command that is an alias keeps the name of the path it aliases (`rename` is `_symbol_rename`)
...
- Read commands default to text; mutations print a compact text status line by default (`--format json --summary` for machine-readable status, `--format json` for full detail)
