...
`src/bn_agent_bridge/paths.py`, `version.py`, `proc_identity.py`, `socket_evidence.py`, `target_hint.py` and `mutation_statuses.py` are symlinks to `src/bn/`, so the bridge and CLI agree on filesystem layout, version, process identity, socket evidence, the multi-target hint grammar and the mutation failure-status vocabulary without duplication. `mutation_statuses.py` is the newest of these and exists for the same reason as the rest (#777): the bridge PRODUCES the failure statuses and `bn.formatters` CLASSIFIES them, so two independently-maintained copies drifted — the bridge's revert gate covered two of the five. `bn.formatters.FAILED_MUTATION_STATUSES` re-exports it, so that documented name still resolves.
