...
- Exit codes: 0 = success, 1 = CLI-side handler error (e.g. partial `session start` failure), 2 = `BridgeError` (transport failures and bridge-side errors, including a status in `FAILED_MUTATION_STATUSES` on a read/resolver call, and a mutation result this CLI cannot classify — malformed or newer than the CLI), 3 = a `_mutate`-marked call whose status is `verification_failed`, `unsupported`, `invalid_request`, `rollback_failed`, or `internal_error` — the refusal is exit 3 whether it was raised up front or during apply, never 2 for a mutation — 4 = a `_mutate`-marked call whose compact summary reports `measured: false`, i.e. the op returned no `results[]` rows to derive counts from AND registered no summary of its own to count with, so the outcome could not be verified (#715); an op that counts through its own registered summary (`go rename`) stays measured and exits 0. A failure still wins over 4 (3 before 4), and a measured all-`noop` is 0
